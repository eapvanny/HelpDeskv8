...
redips.init = function () {
	...
	rd.hover.colorTd = '#414044';	// set hover color
	...
	rd.event.dropped = function () {
		...
		var	objOld = rd.objOld,					// original object
			...
			i, objNew;
		var cell = targetCell.cellIndex;
		var row = targetCell.parentNode.rowIndex;
		var sub_id = targetCell.firstChild.dataset.sub;

		myData[row+'_'+cell] = {
			'sub_id':sub_id
		};
		console.log(myData);// local variables
...
redips.save = function (e) {
	e.preventDefault();
	var class_id_filter = document.getElementById('class_id_filter');
	var class_id = class_id_filter.options[class_id_filter.selectedIndex].value;
	// scan timetable content
	var content = REDIPS.drag.saveContent('table2');
	// and save content
	// window.location.href = url + '?' + content;
	if(content){
		window.location.href = "/academic/timetable?class="+class_id+'&'+content;
	}else {
		alert("No data!")
	}
...
redips.reportButton = function () {
	var	id,			// element id
		...
		// prepare subjectsredips.reportButton();
		subject = {'en': 0, 'ph': 0, 'ma': 0, 'bi': 0, 'ch': 0, 'it': 0, 'ar': 0, 'hi': 0, 'et': 0};
	// loop goes through all collected elements
	for (i = 0; i < redips.divNodeList.length; i++) {
		// define only first two letters of ID
		// (cloned elements have appended c1, c2, c3 ...)
		id = redips.divNodeList[i].id;
